...
const int MHZ14A_PREHEATING_TIME = 3 * 60 ;
const int MHZ19B_PREHEATING_TIME = 3 * 60 ;
...
boolean MHZ::isPreHeating() {
  if (_type == MHZ14A) {
    return ( millis() / 1000) < (MHZ14A_PREHEATING_TIME);
  } else if (_type == MHZ19B) {
    return millis() / 1000 < (MHZ19B_PREHEATING_TIME);
  } else {
...
boolean MHZ::isReady() {
  ...
  if (_type == MHZ14A)
    return (millis() / 1000) - lastRequest < MHZ14A_RESPONSE_TIME;
  else if (_type == MHZ19B)
    return (millis() / 1000) - lastRequest < MHZ19B_RESPONSE_TIME;
  else {
